...
public:
  ...
  emscripten::val jnt_qposadr() const {
    return val(typed_mMuJoCoemory_view(m->njnt * 1, m->jnt_dofadr));
  }
  ...
  emscripten::val jnt_solimpMuJoCoemory_view(m->njnt * 3, m->jnt_axis));
...
emscripten::val dof_armature() const {
  ...
  emscripten::val dof_dampingMuJoCoew(m->nv * 1, m->dof_invweight0));
  }
  ...
  }MuJoCoew(m->ngeom * 1, m->geom_dataid));
  }
  ...
  emscripten::val geom_solmix() cMuJoCo emscripten::val geom_rbound() const {
  ...
  emscripten::val site_bodyid() const {
    return val(typed_memory_view(m - MuJoCo > nsite * 1, m->site_bodyid));
  }
  ...
  emscripten::val site_quat() const {
    return val(typed_memory_view(m->nsite * 4,MuJoCo
...
private:
  ...
  public:
    ...
    // d->qpos otherwise
    //  d->qpos written only if flg_paused and subtree root for selected body
    //  has free joint
    ...
    val mocap_pos() const {
      return val(typed_memory_view(_model->ptr()->nmocap * 3,
                                   _state->ptr()->mocap_pos));
    }
    ...
    val geom_xpos() const {
      return val(typed_memory_view(_model->ptr()->ngeom * 3,
                                   _state->ptr()->geom_xpos));
    }
    val geom_xmat() const {
      return val(typed_memory_view(_model->ptr()->ngeom * 9,
                                   _state->ptr()->geom_xmat));
    }
    val site_xpos() const {
      return val(typed_memory_view(_model->ptr()->nsite * 3,
                                   _state->ptr()->site_xpos));
    }
    val site_xmat() const {
      return val(typed_memory_view(_model->ptr()->nsite * 9,
                                   _state->ptr()->site_xmat));
    }
    val cam_xpos() const {
      return val(
    ...
    val wrap_xpos() const {
      return val(typed_memory_view(_model->ptr()->nwrap * 6,
                                   _state->ptr()->wrap_xpos));
    }
    ...
    val qDeriv() const {
      return val(
          typed_memory_view(_model->ptr()->nD * 1, _state->ptr()->qDeriv));
    }
    val qLU() const {
    ...
    void forwardSkip(int skipstage, int skipsensor) {
      return mj_forwardSkip(_model->ptr(), _state->ptr(), skipstage,
                            skipsensor);
    }
    void inverseSkip(int skipstage, int skipsensor) {
      return mj_inverseSkip(_model->ptr(), _state->ptr(), skipstage,
                            skipsensor);
    }
    ...
    void printFormattedData(std::string filename, std::string float_format) {
      return mj_printFormattedData(_model->ptr(), _state->ptr(),
                                   filename.c_str(), float_format.c_str());
    }
    ...
    void fwdActuation() {
      return mj_fwdActuation(_model->ptr(), _state->ptr());
    }
    ...
    void transmission() {
      return mj_transmission(_model->ptr(), _state->ptr());
    }
    ...
    void _error_i(std::string msg, int i) {
      return mju_error_i(msg.c_str(), i);
    }
    ...
    void _zero(val res, int n) {
      return mju_zero(reinterpret_cast<mjtNum *>(res["byteOffset"].as<int>()),
                      n);
    }
    ...
    mjtNum _sum(val vec, int n) {
      return mju_sum(reinterpret_cast<mjtNum *>(vec["byteOffset"].as<int>()),
                     n);
    }
    ...
    void _addToScl(val res, val vec, mjtNum scl, int n) {
      return mju_addToScl(
          reinterpret_cast<mjtNum *>(res["byteOffset"].as<int>()),
          reinterpret_cast<mjtNum *>(vec["byteOffset"].as<int>()), scl, n);
    }
    void _addScl(val res, val vec1, val vec2, mjtNum scl, int n) {
      return mju_addScl(
          reinterpret_cast<mjtNum *>(res["byteOffset"].as<int>()),
          reinterpret_cast<mjtNum *>(vec1["byteOffset"].as<int>()),
          reinterpret_cast<mjtNum *>(vec2["byteOffset"].as<int>()), scl, n);
    }
    ...
    mjtNum _norm(val res, int n) {
      return mju_norm(reinterpret_cast<mjtNum *>(res["byteOffset"].as<int>()),
                      n);
    }
    ...
    void _eye(val mat, int n) {
      return mju_eye(reinterpret_cast<mjtNum *>(mat["byteOffset"].as<int>()),
                     n);
    }
    ...
    void _transitionFD(mjtNum eps, mjtByte centered, val A, val B, val C,
                       val D) {
  ...
  EMSCRIPTEN_BINDINGS(mujoco_wasm) {
    ...
    enum_<mjtConstraint>("mjtConstraint")
        ...
        .value("mjCNSTR_FRICTION_TENDON",
               mjtConstraint ::mjCNSTR_FRICTION_TENDON)
    ...
    class_<Simulation>("Simulation")
        ...
        .function("differentiatePos", &Simulation::differentiatePos,
                  allow_raw_pointers())
        .function("integratePos", &Simulation::integratePos,
                  allow_raw_pointers())
    ...
    value_object<mjvPerturb>("mjvPerturb")
        ...
        .field("active",
               &mjvPerturb::active) // perturbation bitmask (mjtPertBit)
        .field(
            "active2",
    ...
    value_object<mjContact>("mjContact")
        .field("dist",
               &mjContact::dist) // distance between nearest points; neg:
                                 // penetration
        .field("pos",
               &mjContact::pos) // position of contact point: midpoint between
                                // geoms
        .field("frame", &mjContact::frame) // normal is in [0-2]
        .field("includemargin",
               &mjContact::includemargin) // include if
                                          // dist<includemargin=margin-gap
        ...
        .field("dim",
               &mjContact::H) // contact space dimensionality: 1, 3, 4 or 6
        .field("geom1", &mjContact::H)         // id of geom 1
        .field("geom2", &mjContact::H)         // id of geom 2
        .field("exclude", &mjContact::exclude) // 0: include, 1: in gap, 2:
                                               // fused, 3: equality, 4: no dofs
        .field("efc_address",
    ...
    value_object<mjLROpt>("mjLROpt")
        ...
        .field("timeconst",
               &mjLROpt::timeconst) // time constant for velocity reduction; min
                                    // 0.01
        .field(
            "timestep",
        ...
        .field("inteval",
               &mjLROpt::inteval) // evaluation time interval (at the end)
        .field("tolrange",
    ...
    value_object<mjOption>("mjOption")
        ...
        .field("impratio",
               &mjOption::impratio) // ratio of friction-to-normal contact
                                    // impedance
        ...
        .field("mpr_tolerance",
               &mjOption::mpr_tolerance) // MPR solver tolerance
        ...
        .field("collision",
               &mjOption::collision)    // collision mode (mjtCollision)
        .field("cone", &mjOption::cone) // type of friction cone (mjtCone)
        .field("jacobian",
               &mjOption::jacobian)         // type of Jacobian (mjtJacobian)
        ...
        .field("noslip_iterations",
               &mjOption::noslip_iterations) // maximum number of noslip solver
                                             // iterations
        .field("mpr_iterations",
               &mjOption::mpr_iterations) // maximum number of MPR solver
                                          // iterations
        .field("disableflags",
               &mjOption::disableflags) // bit flags for disabling standard
                                        // features
        .field(
            "enableflags",
